...
% TODO Definition verlinken
% TODO "AliveAnalyzer" beschreiben (Java Class in javac OpenJDK)
...
\begin{itemize}
	% TODO Eclipse compiler untersuchen
...
void duplicateConditions(int s_operationHour, int m_operationHour) {
	...
	% TODO Stopped here
	\caption{Even though these warnings do not hint to the unreachable code directly, they show a code smell prone to errors. In this particular case the else branch of the second if-then-else block cannot be reached, since the method already returned. In case one of the variables used in the second said condition, this error would not occur! Removing the return statement in line 7 removes this type of problem, but the second condition would always evaluate to false. }
...
\section{Joogie}
\label{sec:sca paper}
% TODO: Verbessern und Beispiele hinzufügen
